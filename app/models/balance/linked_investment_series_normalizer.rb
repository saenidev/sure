class Balance::LinkedInvestmentSeriesNormalizer
  ...
  # Transaction-era history is only trustworthy when holdings can be
  # reconstructed backward from trades (see common_supported_history_start_date).
  # Without trades, buys/sells arrive as plain cash transactions, so the
  # stable provider snapshot window is the earliest honest start.
