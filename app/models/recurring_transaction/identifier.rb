  ...
  class Identifier
    ...
    private
      ...
      def collect_patterns(min_occurrences:)
        ...
        # Skip transfer-kind and ledger-only transactions: transfer rows are one half of a
        # Transfer pair, so grouping them under their single account would produce incoherent
        # recurring "patterns" (recurring transfers are tracked on a different shape, with
        # destination_account_id), and ledger-only rows such as generated debt interest are
        # synthetic balance activity. Filtering at the SQL level avoids loading and discarding
        # ineligible entries for a busy family.
