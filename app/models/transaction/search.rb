class Transaction::Search
  ...
  def totals
    @totals ||= begin
      # v4: the Uncategorized filter's exclusion set changed (upstream #2592
      # let loan_payment / investment_contribution in; ledger-only debt
      # interest stays out). Without a bump, a totals entry cached under the
      # old logic would keep being served (same cache_key_base) after deploy,
      # disagreeing with the (uncached) transactions_scope list until
      # entries_cache_version next changes for that family.
