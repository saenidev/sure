...
class InvestmentStatement
  ...
  def current_holdings
    @current_holdings ||= Holding::CurrentForInvestmentAccounts
      .new(current_holdings_account_ids)
      .relation
  ...
  private
    ...
    # A linked account whose whole balance is cash holds no positions, however
    # many provider holding rows it still carries from an earlier sync (see
    # Account#all_cash_portfolio?). An unlinked account with a stale zero
    # balance is left in: its holdings rows are the only record of its
    # positions.
    def current_holdings_account_ids
      ids = investment_account_ids
      return ids if ids.empty?

      # Mirrors Account#all_cash_portfolio?: a zero balance is not "all cash".
      all_cash_linked_ids = Account
        .where(id: ids)
        .where("accounts.balance = accounts.cash_balance")
        .where.not(cash_balance: 0)
        .where("EXISTS (SELECT 1 FROM holdings provider_rows WHERE provider_rows.account_id = accounts.id AND provider_rows.account_provider_id IS NOT NULL)")
        .pluck(:id)

      ids - all_cash_linked_ids
    end
