class PagesController < ApplicationController
  ...
  # Sections whose rendered output depends only on the dashboard fragment
  # cache key (period + family/account/entry/holding versions). Sections with
  # their own request params (money flow / spending trend month and account
  # filters) or data outside those versions (insights) render uncached.
  CACHED_DASHBOARD_SECTIONS = %w[cashflow_sankey outflows_donut investment_summary net_worth_chart balance_sheet].freeze
  ...
  def dashboard
    ...
    # The money flow and spending trend widgets take their own month and
    # account filters, which the period-keyed chart cache does not capture,
    # so they are built per request (and their sections are not fragment
    # cached; see CACHED_DASHBOARD_SECTIONS).
