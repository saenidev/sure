...
class AccountTest < ActiveSupport::TestCase
  ...
  # A zero balance is not "all cash": an emptied or not-yet-valued provider
  # account must still show its latest snapshot, not nothing.
  test "current_holdings keeps the latest provider snapshot when balance and cash are both zero" do
    account = @family.accounts.create!(
      owner: @admin,
      name: "Unvalued Exchange",
      balance: 0,
      cash_balance: 0,
      currency: "USD",
      accountable: Investment.new
    ...
    security = Security.create!(ticker: "ZEROBAL", name: "Zero Balance Co")
    account.holdings.create!(
    ...
    assert_equal [ "ZEROBAL" ], account.current_holdings.map { |h| h.security.ticker }
  end
