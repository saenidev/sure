...
class InvestmentStatementTest < ActiveSupport::TestCase
  ...
  # Account#all_cash_portfolio? treats balance == cash_balance == 0 as "not
  # valued", not "all cash"; the dashboard must agree with the account page.
  test "current_holdings keeps a linked account whose balance and cash are both zero" do
    account = create_investment_account(balance: 0, cash_balance: 0)
    provider = AccountProvider.create!(account: account, provider: plaid_accounts(:one))
    security = Security.create!(ticker: "ZEROSTMT", name: "Zero Statement Co")
    account.holdings.create!(
      security: security, date: Date.current, qty: 2, price: 50, amount: 100,
      currency: "USD", account_provider: provider
    )

    assert_equal account.current_holdings.map(&:id).sort, @statement.current_holdings.map(&:id).sort
    assert_equal [ "ZEROSTMT" ], @statement.current_holdings.map { |h| h.security.ticker }
  end
